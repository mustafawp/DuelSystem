...
local pg,pu = 508,263 -- panelGenislik, panelUzunluk // windowWidth, windowHeight
local x,y = (sx-pg)/2, (sy-pu)/2 -- panel ortalama

guipanel = guiCreateWindow(x, y, 508, 263, "Düello Paneli", false)
guiWindowSetSizable(guipanel, false)
guiSetVisible(guipanel,false)

gridlist = guiCreateGridList(9, 26, 209, 221, false, guipanel)
column = guiGridListAddColumn(gridlist, "Çevrimiçi Oyuncular", 0.9)
isimlabel = guiCreateLabel(235, 49, 94, 21, "Oyuncu Nick", false, guipanel)
guiSetFont(isimlabel, font31)
oyuncunick = guiCreateEdit(339, 43, 152, 31, "", false, guipanel)
bahislabel = guiCreateLabel(235, 96, 94, 21, "Bahis Miktar", false, guipanel)
guiSetFont(bahislabel, font31)
bahismiktar = guiCreateEdit(339, 90, 152, 31, "", false, guipanel)
meydanoku = guiCreateButton(292, 143, 154, 47, "Meydan Oku!", false, guipanel)
haritasec = guiCreateButton(229, 200, 120, 44, "Harita Seç", false, guipanel)
duellogecmisi = guiCreateButton(371, 200, 120, 44, "Düello Geçmiş", false, guipanel)

-- harita window

local pgs,pus = 508,263 -- panelGenislik, panelUzunluk // windowWidth, windowHeight
local xa,ya = (sx-pgs)/2, (sy-pus)/2 -- panel ortalama

haritawindow = guiCreateWindow(xa, ya, 431, 202, "Düello Haritası", false)
guiWindowSetSizable(haritawindow, false)
guiSetVisible(haritawindow,false)

randomharita = guiCreateRadioButton(159, 33, 132, 15, "Random Harita", false, haritawindow)
guiSetFont(randomharita, font31)
guiRadioButtonSetSelected(randomharita, true)
haritaiki = guiCreateRadioButton(184, 74, 77, 15, "Harita 2", false, haritawindow)
guiSetFont(haritaiki, font31)
haritabir = guiCreateRadioButton(73, 74, 75, 15, "Harita 1", false, haritawindow)
guiSetFont(haritabir, font31)
haritauc = guiCreateRadioButton(291, 74, 94, 15, "Harita 3", false, haritawindow)
guiSetFont(haritauc, font31)
haritadort = guiCreateRadioButton(126, 109, 75, 15, "Harita 4", false, haritawindow)
guiSetFont(haritadort, font31)
haritabes = guiCreateRadioButton(240, 109, 75, 15, "Harita 5", false, haritawindow)
guiSetFont(haritabes, font31)
haritakaydet = guiCreateButton(218, 145, 107, 38, "Kaydet", false, haritawindow)
guiSetFont(haritakaydet, font31)
haritageri = guiCreateButton(101, 145, 107, 38, "Geri", false, haritawindow)
guiSetFont(haritageri, font31)    

-- Duello Geçmişi window

local pga,pua = 358,237 -- panelGenislik, panelUzunluk // windowWidth, windowHeight
local xs,ys = (sx-pga)/2, (sy-pua)/2 -- panel ortalama

gecmiswindow = guiCreateWindow(xs, ys, 594, 319, "Düello Geçmişin", false)
guiWindowSetSizable(gecmiswindow, false)
guiSetVisible(gecmiswindow,false)

gecmisliste = guiCreateGridList(12, 29, 572, 250, false, gecmiswindow)
guiGridListAddColumn(gecmisliste, "Nick'in", 0.2)
guiGridListAddColumn(gecmisliste, "Rakip", 0.2)
guiGridListAddColumn(gecmisliste, "Sonuç", 0.2)
guiGridListAddColumn(gecmisliste, "Durum", 0.2)
guiGridListAddColumn(gecmisliste, "Tarih", 0.2)
--guiGridListAddRow(gecmisliste])
--guiGridListSetItemText(gecmisliste, 0, 1, "-", false, false)
--guiGridListSetItemText(gecmisliste, 0, 2, "-", false, false)
gecmisgeri = guiCreateButton(32, 284, 65, 25, "Geri", false, gecmiswindow)    

bindKey("F7","down",function()
	 local durum = guiGetVisible(guipanel) 
     if durum == false then
	    guiSetText(oyuncunick, "")
		Refresh()
		guiSetVisible(guipanel, true) 
		showCursor(true)
	 else
	 	guiSetVisible(guipanel, false) 
		showCursor(false)
	 end
	 if guiGetVisible(haritawindow) == true then
	 guiSetVisible(haritawindow,false)
	 end
	 if guiGetVisible(gecmiswindow) == true then
	 guiSetVisible(gecmiswindow,false)
	 end
end)

addEventHandler("onClientGUIClick", root, function()
  local player = guiGridListGetItemData(gridlist, guiGridListGetSelectedItem(gridlist), 1)
  if source == gridlist then
    guiSetText(oyuncunick, player)
  elseif source == haritasec then
	guiSetVisible(guipanel,false)
	guiSetVisible(haritawindow,true)
  elseif source == duellogecmisi then
	guiSetVisible(guipanel,false)
	guiGridListClear(gecmisliste)
	triggerServerEvent("gecmisilistele",localPlayer)
	guiSetVisible(gecmiswindow,true)
  end

  if source == gecmisgeri then
	guiSetVisible(gecmiswindow,false)
	guiSetVisible(guipanel,true)
  elseif source == haritageri then
	guiSetVisible(haritawindow,false)
	guiSetVisible(guipanel,true)
  end

  if source == haritakaydet then
	exports.hud:dm("#ffffffHarita ayarları #ff7f00Başarıyla #ffffffkaydedildi.",255,127,0,true)
  end
...
addEventHandler("onClientGUIClick", root, function()
	local map
	if guiRadioButtonGetSelected(randomharita) then
		map = "random"
	elseif guiRadioButtonGetSelected(haritabir) then
		map = "harita1"
	elseif guiRadioButtonGetSelected(haritaiki) then
		map = "harita2"
	elseif guiRadioButtonGetSelected(haritauc) then
		map = "harita3"
	elseif guiRadioButtonGetSelected(haritadort) then
		map = "harita4"
	elseif guiRadioButtonGetSelected(haritabes) then
		map = "harita5"
	end
  if source == meydanoku then
  local c = "duel"
  local challenger = guiGetText(oyuncunick)
  local bahis = guiGetText(bahismiktar)
  if tonumber(bahis) < tonumber(0) then exports.hud:dm("- 'li sayı girmek yasaktır.",true) return end
  if challenger == "" then exports.hud:dm("Oyuncu ismi giriniz",true) return end
  if bahis == "" then exports.hud:dm("bahis miktarı giriniz",true) return end
  guiSetVisible(guipanel, false) 
  showCursor(false)
  triggerServerEvent("duellobaslat", root, localPlayer, c, challenger, bahis,map)
  end
end)
...
addEventHandler('onClientPlayerStartDuel',root,
    function()
        g_currentDuel = true
    end
)

addEventHandler('onDuelEnded',root,
    function()
        g_currentDuel = nil
    end
)
    ...
    function ( )
        if getElementType( source ) == "vehicle" then
            if g_currentDuel == true then
                triggerServerEvent(getResourceName(getThisResource())..":destroyVehicleInDuel",localPlayer,source)
            end
        end
    end
)
...
addEvent("gecmislistes",true)
addEventHandler("gecmislistes",root,function(veriler)
	for i,v in pairs(veriler) do
	row = guiGridListAddRow(gecmisliste,v["nicki"],v["rakip"],v["sonuc"],v["durum"],v["tarih"])
	end
end)
    ...
    function()
        if c_duelTimer and isTimer(c_duelTimer) then
            ...
            if remaining then
                ...
                dxDrawText("Raund Başlıyor! İyi şanslar..", guiX*494, guiY*332, guiX*786, guiY*390, tocolor(255, 255, 255, 255), 1.50, "default-bold", "center", "center", false, false, false, false, false)
...
clientsideResourceStart = function()
  if column then
    for id,playeritem in ipairs(getElementsByType("player")) do
      ...
      guiGridListSetItemText(gridlist, row, column, (string.gsub (getPlayerName(playeritem), "#%x%x%x%x%x%x", "")), false, false)
	  guiGridListSetItemData(gridlist,row,column, getPlayerName(playeritem))
    end
  end
end
...
function search()
    ...
    local text = guiGetText(oyuncunick)
    ...
    if (text == "") then
        ...
        for index, player in ipairs(getElementsByType("player")) do
            ...
            if (player ~= localPlayer) then
                ...
                guiGridListSetItemText(gridlist, row,1,(string.gsub (getPlayerName(player), "#%x%x%x%x%x%x", "")), false, false)
				
				guiGridListSetItemData(gridlist,row,1, getPlayerName(player))
...
addEventHandler("onClientGUIChanged", oyuncunick, search)